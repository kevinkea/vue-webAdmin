<template>
	<div class="app-container calendar-list-container">
		...
		<el-table stripe :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
			...
			<el-table-column :label="'会籍状态'">
				<template slot-scope="scope">
					<span>{{scope.row.chs_mbr_status}}</span>
				</template>
			</el-table-column>
			<el-table-column :label="'教练'">
				<template slot-scope="scope">
					<span>{{scope.row.chs_trainer}}</span>
				</template>
			</el-table-column>
			<el-table-column :label="'课程'">
				<template slot-scope="scope">
					<span>{{scope.row.chs_lesson_id}}</span>
				</template>
			</el-table-column>
			<el-table-column :label="'课程状态'">
				<template slot-scope="scope">
					<span>{{scope.row.chs_lesson_status}}</span>
				</template>
			</el-table-column>
			<el-table-column :label="'操作'" class-name="small-padding fixed-width">
				<template slot-scope="scope">
					<el-button type="primary" size="medium" @click="handleUpdate(scope.row)">{{'私教签署'}}</el-button>
					</el-button>
				</template>
			</el-table-column>
		</el-table>
...
<script>
	import { fetchSignPaid } from '@/api/pt'
	  ...
	  methods: {
	    getList() {
	      this.listLoading = true
		    fetchSignPaid(this.listQuery).then(response => {
